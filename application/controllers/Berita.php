...
class Berita extends CI_Controller
{
	...
	public function index()
	{
		...
		if (is_numeric($dari)) {
			if ($this->input->post('keyword') != '') {
				...
				$data['title'] = "Hasil Pencarian - " . cetak($keyword);
				$data['description'] = description();
				$data['keywords'] = keywords();
				$data['berita'] = $this->model_utama->cari_berita($keyword);
				...
				$data['berita'] = $this->model_utama->view_join_where('berita', 'users', 'kategori', 'username', 'id_kategori', 'id_berita', 'DESC', $dari, $config['per_page'],'terbit','Y');
	...
	// {
	// 	$jumlah = $this->model_utama->view('berita')->num_rows();
	// 	$config['base_url'] = base_url() . 'berita/ListLelang/';
	// 	$config['total_rows'] = $jumlah;
	// 	$config['per_page'] = 15;
	// 	if ($this->uri->segment('3') == '') {
	// 		$dari = 0;
	...
	// 			$data['description'] = description();
	// 			$data['keywords'] = keywords();
	// 			$data['berita'] = $this->model_utama->view_joinn_2('berita', 'users', 'kategori', 'username', 'id_kategori', array('berita.id_kategori' => 62), 'id_berita', 'DESC', $dari, $config['per_page']);
	// 			$this->pagination->initialize($config);
	// 		}
	// 	} else {
	// 		redirect('main');
	...
	// 			$data['description'] = description();
	// 			$data['keywords'] = keywords();
	// 			$data['berita'] = $this->model_utama->view_more_like('berita','users','kategori','username','id_kategori','id_berita','DESC',$dari,$config['per_page']);
	// 			$this->pagination->initialize($config);
	// 		}
	// 	}else{
	// 		redirect('main');
