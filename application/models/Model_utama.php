...
class Model_utama extends CI_model {
    ...
    public function view_order_by($table, $order_by, $ordering){
        $this->db->select('*');
        $this->db->from($table);
    ...
    public function view_like($table1, $table2, $table3, $field, $field1, $where, $order, $ordering, $baris, $dari)
    {
        $this->db->select('*');
        $this->db->from($table1);
    ...
    public function view_more_like($table1, $table2, $table3, $field, $field1, $order, $ordering, $baris, $dari)
    {
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1 . '.' . $field . '=' . $table2 . '.' . $field);
        $this->db->join($table3, $table1 . '.' . $field1 . '=' . $table3 . '.' . $field1);
        $this->db->where_in('berita.id_kategori', array(64, 65));
        $this->db->order_by($order, $ordering);
        $this->db->limit($dari, $baris);
        return $this->db->get();
    }
    ...
    public function view_where_2($table, $data)
    {
        $this->db->where($data);
    ...
    public function view_orderby($table, $data, $order, $ordering)
    {
        $this->db->where($data);
        $this->db->order_by($order, $ordering);
        return $this->db->get($table);
    ...
    public function view_table($table, $order, $ordering, $baris, $dari)
    {
        $this->db->order_by($order, $ordering);
        $this->db->limit($dari, $baris);
        return $this->db->get($table);
    ...
    public function select_where_limit($table, $order, $ordering, $baris, $dari, $a, $b){
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where($table . '.' .$a, $b);
        $this->db->order_by($order, $ordering);
        $this->db->limit($dari, $baris);
        return $this->db->get();
    }

    public function view_join_where($table1, $table2, $table3, $field, $field1, $order, $ordering, $baris, $dari, $a, $b)
    {
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1 . '.' . $field . '=' . $table2 . '.' . $field);
        $this->db->join($table3, $table1 . '.' . $field1 . '=' . $table3 . '.' . $field1);
        $this->db->where($table1 . '.' .$a, $b);
        $this->db->order_by($order, $ordering);
        $this->db->limit($dari, $baris);
        return $this->db->get();
    }

    public function view_join_where_not_limit($table1, $table2, $table3, $field, $field1, $order, $ordering, $a, $b)
    {
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1 . '.' . $field . '=' . $table2 . '.' . $field);
    ...
    public function view_joinn_2($table1, $table2, $table3, $field, $field1, $where, $order, $ordering, $baris, $dari)
    {
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1 . '.' . $field . '=' . $table2 . '.' . $field);
    ...
    function cari_berita($kata){
        ...
        $cari = "SELECT * FROM berita a join users b on a.username=b.username
                    join kategori c on a.id_kategori=c.id_kategori
                       WHERE a.terbit='Y' AND";
        for ($i = 0; $i <= $jml_kata; $i++) {
            // tambah $this->db->escape_like_str
            $cari .= " a.judul LIKE '%" . $this->db->escape_like_str($pisah_kata[$i]) . "%'";
            if ($i < $jml_kata) {
                $cari .= " OR ";
            }
        }
        // $cari .= " ORDER BY a.id_berita DESC LIMIT 15";
        $cari .= " ORDER BY a.id_berita DESC";
