...
<?php
foreach ($rows->result_array() as $rows) {
          ?>
                  <div class="blog-details-area wow fadeInUp">
                      <?php if ($rows['gambar'] == '') {
                            echo "<img src='" . base_url() . "asset/img_album/no-image.jpg' alt='no-image.jpg'/>";
                        } else { ?>
                          <img src="<?php echo base_url() . "asset/foto_pengumuman/" . $rows['gambar'] ?>" alt="<?php echo $rows['judul'] ?>">
                      <?php } ?>

                      <h2 class="post-title"><?php echo $rows['judul']; ?></h2>

                      <div class="post-meta">
                          <a href="disable"><i class="fa fa-calendar"></i> <?php echo tgl_indo($rows['tgl_posting']); ?></a>
                  ...
                  <div class="section-heading" style="margin-bottom: 30px;">
                      <h5>PENGUMUMAN TERKAIT</h5>
                      <div class="line" style="width: 40px;"></div>
                  </div>

                  <?php
                    $no = 1;
                    $sb1 = $this->model_utama->view_ordering_limit('pengumuman', 'id_pengumuman', 'DESC', 1, 5);
                    ...
                    ?>
                      ...
                      <div class="single-news-area d-flex align-items-center wow fadeInRight" data-wow-delay="0.<?php echo $no; ?>s">
                          <div class="blog-thumbnail">
                              <?php if ($r['gambar'] != '') { ?>
                                  <img src="<?php echo base_url() ?>asset/foto_pengumuman/<?php echo $r["gambar"] ?>" alt="" style="height: 70px; width: 70px;">
                              <?php } else { ?>
                                  <img src="<?php echo base_url(); ?>asset/img_album/no-image.jpg" alt="" style="height: 70px; width: 70px;">
                              <?php } ?>

                          </div>
                          <div class="blog-content">
                              <a href="<?php echo base_url('pengumuman/detail/' . $r['judul_seo']) ?>" class="post-title"><?php echo character_limiter($r['judul'], 40); ?></a>
                              <span class="post-date"><i class="fa fa-calendar"></i> <?php echo tgl_indo($r['tgl_posting']); ?> </span>
                          </div>
                      </div>
                      <?php } ?>
                  <?php $no++;
